        ...
        <div className={classes.container}>
            <div className={classes.commonStyle}>
                ...
                {trades.map((trade: IDisplayTransaction) => {
                    return <div className={classes.transactionContainer} key={trade.timestamp}>
                        <div className={classes.left}>
                        <div>{trade.qty}</div>
                        <div className={classes.timeStamp}>{trade.timestamp.split('(')[0]}</div>

                        </div>
                        <div className={classes.right + " " + (trade.type=="Buy"?classes.buy:classes.sell)}>{trade.type}</div>
                    </div>;
                })}
            </div>
            <div className={classes.commonStyle}>
